...
### Bug #1：对话页发送消息无效（待修复）
- **时间**：Phase 9 第一轮线上测试
- **现象**：在 /chat 页面输入文字后点击发送按钮，消息不会出现在聊天区域，输入框文字不清空，AI 无响应
- **优先级**：P0（核心功能完全不可用）
- **状态**：待修复

### Bug #2：美食页"必点菜品"和"注意事项"下方内容为空（待验证）
- **时间**：Phase 9 第一轮线上测试
- **现象**：snapshot 中美食卡片只显示标题"必点菜品"和"注意事项"，没有看到具体内容
- **优先级**：P1
- **状态**：待验证（可能是 snapshot 截断，需要截图确认）
